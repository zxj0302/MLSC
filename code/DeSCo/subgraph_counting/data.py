...
def load_data(
    ...
    elif dataset_name == "RWDataset":
        dataset = RWDataset(root=save_dir, transform=transform)
    elif re.match(r"Set_\d+$", dataset_name):
        dataset = RWDataset(root=os.path.join(root_folder, dataset_name, sub_folder), transform=transform)
    else:
        print(dataset_name)
...
class RWDataset(InMemoryDataset):
    '''
    real-world dataset sampled from TUDataset and OGB
    '''
