            ...
            for line in f_input: # Read line by line. This is more memory efficient, but might be slower
                if first_line:   # ignore first line
                    ...
                    if not line.startswith('#'): # Skip comments
                        ...
                        for node_num in range(0, int(tokens[0])):
                            self.vertices.add(str(node_num))        # Add node1 to vertices
                            self.adj_list[str(node_num)] = set()  # Initialize node1's list to have node2
                            self.degrees[str(node_num)] = 0 
                        first_line = False
                        continue
                line = line.strip() # Remove whitespace from edge
                if not line.startswith('#'): # Skip comments
