...
# Saves the mapping from the original vertex IDs to the ID of vertices in the sanitized version in  filename_sanitization_map.json
#
...
#             continue
    ...
    json_dump = json.dumps(mapping)
    out_name = names[0] + '_sanitization_map.json'  # Output file name
    f_output = open(out_name,"w")
    f_output.write(json_dump)
    f_output.close()
