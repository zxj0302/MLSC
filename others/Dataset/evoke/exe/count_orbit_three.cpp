...
// We want to count node orbits and edge orbits in patters of size 3, 2, and 1.

int main(int argc, char *argv[])
{
  int pattern_size = 3;
  int num_orbits = 4; // number of different node orbits in all the patterns
  int num_edge_orbits = 2; // number of different edge orbits in all the patterns
  ...
  *
  ...
  // OrbitInfo orbit_counts = newOrbitInfo(&(dag.outlist), num_node_patterns, num_edge_patterns);
