...
{
  Graph g;
  double Cstart, Cend, TimeArray[20];
...
//   (dag.inlist).print();
  ...
  Cend=clock();
  TimeArray[0]=Ctimediff(Cstart,Cend);
...
// 
  ...
  Cend=clock();
  TimeArray[1]=Ctimediff(Cstart,Cend);
  ...
  Cend=clock();
  TimeArray[2]=Ctimediff(Cstart,Cend);
  ...
  Cend=clock();
  TimeArray[3]=Ctimediff(Cstart,Cend);
  ...
  { 
    int i;
    double x,y, wcnt, wiocnt, woocnt;
    
    woocnt=wiocnt=wcnt=0;
    for (VertexIdx i = 0; i < cg_relabel.nVertices; i++) {
      x = ( double)(dag.outlist).offsets[i+1]-(dag.outlist).offsets[i];
      y = ( double)(dag.inlist).offsets[i+1]-(dag.inlist).offsets[i];
      woocnt +=x*(x-1)/2;
      wiocnt+= x*y;
      wcnt+=(x+y)*(x+y-1)/2;   
 }
    ...
    out_file.open("counts.txt",std::ios::app);
    out_file << argv[1] << " ";
    out_file << cg_relabel.nVertices << " " << cg_relabel.nEdges << " ";
    out_file << woocnt << " "<< wiocnt<< " " << wcnt << " " << tri_info.total << " ";
    ...
    out_file.open("times.txt",std::ios::app);
    out_file << argv[1] << " ";
    for (i=0;i< 8;i++)
      out_file << TimeArray[i] << " ";
    out_file << "\n";
    out_file.close();
  }
